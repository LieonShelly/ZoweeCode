...
class ZEPesonalCenterViewController: ZEBaseViewController,UITableViewDelegate,UITableViewDataSource{
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if ZELoginUserInfo.loginUserInfo().UserToken != nil {
            if ZELoginUserInfo.loginUserInfo().UserName == ""{
                profileHeader.model = ZEPersonalHeader(imgName: ZELoginUserInfo.loginUserInfo().Url, nickname: ZELoginUserInfo.loginUserInfo().Mobile, descWord: "good morning")
            }else{
                profileHeader.model = ZEPersonalHeader(imgName: ZELoginUserInfo.loginUserInfo().Url, nickname: ZELoginUserInfo.loginUserInfo().UserName, descWord: "good morning")
            }
        }else{
            profileHeader.model = ZEPersonalHeader(imgName: "", nickname: "", descWord: "美好的一天从清晨开始")
        }
    }
...
//        title = " "
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        case 3:
            ...
            //消息数量小球
            let lab = ZEMsgCountLabel.init(frame: CGRectMake(118, 22, 13, 13))
            lab.text = "10"
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath){
        ...
        if ZELoginUserInfo.loginUserInfo().UserToken == nil && indexPath.row < 6{
            let SB = UIStoryboard.init(name: "ZPersonalCenter", bundle: nil)
            let loginVC = SB.instantiateViewControllerWithIdentifier("ZELoginViewController")
            THURLNavigation.pushViewController(loginVC, animated: true)
        }else{
            ...
            case 0:
                let SB = UIStoryboard.init(name: "ZPersonalCenter", bundle: nil)
                let oderVC = SB.instantiateViewControllerWithIdentifier("ZEMyOderViewController")
