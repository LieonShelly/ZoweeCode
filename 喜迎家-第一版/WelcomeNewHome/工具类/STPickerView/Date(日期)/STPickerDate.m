...
- (void)setupUI {
    ...
    [self.pickerView selectRow:(_month -[NSCalendar currentMonth]) inComponent:1 animated:NO];
    [self.pickerView selectRow:(_day - [NSCalendar currentDay]) inComponent:2 animated:NO];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    if (component == 0) {
        ...
        return [NSCalendar getMonthWithYear:yearSelected];
    }else {
        NSInteger yearSelected = [pickerView selectedRowInComponent:0] + self.yearLeast;
        NSInteger monthSelected = [pickerView selectedRowInComponent:1] + [NSCalendar getFirstMonthWithYear:yearSelected];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view
{
    ...
    NSString *text;
    if (component == 0) {
        text =  [NSString stringWithFormat:@"%ld年", row + _yearLeast];
    }else if (component == 1){
        text =  [NSString stringWithFormat:@"%ld月", row + [NSCalendar getFirstMonthWithYear:self.year]];
        
    }else{
        text = [NSString stringWithFormat:@"%ld日", row + [NSCalendar getFirstDayWithYear:self.year Month:self.month]];
    }
...
- (void)reloadData
{
    ...
    self.month = [self.pickerView selectedRowInComponent:1] + [NSCalendar getFirstMonthWithYear:self.year];
    self.day   = [self.pickerView selectedRowInComponent:2] + [NSCalendar getFirstDayWithYear:self.year Month:self.month];
}
