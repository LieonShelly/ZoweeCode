...
- (UIButton *)matchBtn
{
    if (_matchBtn == nil) {
        ...
        [_matchBtn setImage:[UIImage imageNamed:@"btn"] forState:UIControlStateNormal];
        [_matchBtn sizeToFit];
...
- (void)saveProject{
    ...
    [[ApplicationManage shareManage] SaveProjectInfoWith:self.orderInfo Block:^(THNetWorkError *error, NSNumber *projectID) {
        ...
        if (error) {
            [self showAllMenu];
            [MBProgressHUD showError:@"数据上传失败" toView:self.view];
            NSLog(@"%@",error.errorDescription);
   ...
   // UIImageWriteToSavedPhotosAlbum(iamgeScreen, self, nil, nil);
    ...
    [[ApplicationManage shareManage] SaveProjectImageWiith:info file:loadFile Block:^(THNetWorkError *error, NSNumber *ID) {
        ...
        if (error) {
            ...
            [MBProgressHUD showSuccess:@"房间数据上传成功,即将退出" toView:self.view];
...
-  (void)updateOrderInfo
{
    ...
    self.roomInfo.RArea =@(self.roomCover.subText.doubleValue * 1000 * 1000) ;
    self.roomInfo.RWindowsCount =@(roomWindowCount) ;
    self.roomInfo.RDoorCount =@(roomDoorCount);
    self.orderInfo.ORoomList = @[ self.roomInfo];
    self.orderInfo.CustomerName = self.mission.CustomerName;
    self.orderInfo.Address = self.mission.Address;
    roomWindowCount = 0;
    roomDoorCount = 0;
    NSDictionary * dict = [self.orderInfo mj_keyValues];
    NSLog(@"JSON:%@",dict);
}
...
- (void)matchBtnClick
{
    ZERecommandViewController * rvc = [[ZERecommandViewController alloc]init];
...
- (void)initUI
{
    ...
    [self setupDrawBoard];
    [self.view insertSubview:self.matchBtn aboveSubview:self.contentView];
    
    self.matchBtn.sd_layout
    .rightSpaceToView(self.matchBtn.superview,50)
    .bottomSpaceToView(self.matchBtn.superview,50)
    .widthIs(self.matchBtn.width)
    .heightIs(self.matchBtn.height);
...
- (void)hiddAllMenu
{
    ...
    self.lazerBtn.hidden = YES;
    self.toolTableView.hidden = YES;
    self.furnitureView.hidden = YES;
...
- (void)showAllMenu
{
    ...
    self.lazerBtn.hidden = NO;
    self.toolTableView.hidden = NO;
    self.furnitureView.hidden = NO;
...
- (void)ceilingHeight
{
    ...
    TYAlertAction * action1 = [TYAlertAction actionWithTitle:@" 确定" style:TYAlertActionStyleDestructive handler:^(TYAlertAction *action) {
        ...
        // 更新模型数据
        weakSelf.roomInfo.RPlatHeight = TwoBitNumber(textField.text.doubleValue * 1000);
    }];
