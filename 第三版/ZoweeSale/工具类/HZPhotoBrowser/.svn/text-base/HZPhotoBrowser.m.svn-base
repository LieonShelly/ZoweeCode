...
@property (nonatomic ,assign) NSInteger THSeletonCount;
@end
...
- (void)viewDidLoad
{
    ...
    _THSeletonCount = _currentImageIndex;
}
...
- (void)showPhotoBrowser
{
    ...
    UIView *sourceView = self.sourceImagesContainerView.subviews[0];//自己修改的代码,为了适合自己的逻辑.
...
- (void)hidePhotoBrowser:(UITapGestureRecognizer *)recognizer
{
    ...
    if (tempImageView.image) {
        ...
        if (tempImageViewH < appHeight) {
            tempImageView.frame = CGRectMake(0, 0, appWidth, appHeight);
        } else {
            tempImageView.frame = CGRectMake(0, 0, appWidth, appHeight);
        }
    } else {
    ...
    if (_THSeletonCount != _currentImageIndex) {
        [self dismissViewControllerAnimated:NO completion:nil];
...
{
    return _isPortraitorLeft;
}

- (UIInterfaceOrientationMask)supportedInterfaceOrientations
{
    if (_isPortraitorLeft) {
