...
#define CancelStrImgaeName @"取消按钮"
...
-(void)initUI
{
    ...
    CGFloat DownY = kH - 45;
    [DownView setTag:10];
    [DownView setFrame:(CGRect){{0,DownY},{kW,45}}];
...
-(void)CirculatingItmes
{
    ...
    for (MenuLabel *Obj in _ItmesArr) {
        ...
        if (_ItmesArr.count == 1) {
            buttonX = (kW - ButtonWidth) / 2;
            buttonY = CGRectGetMaxY(downView.frame) - 80 - ButtonHigh;
        }
