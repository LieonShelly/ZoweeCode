...
+ (NSInteger)getMonthWithYear:(NSInteger)year{
    if (year == [self currentDateComponents].year) {
        return 12-[NSCalendar currentDateComponents].month+1;
    }else{
        return 12;
    }
}
...
{
    NSInteger num = 0;
    if ((year == [self currentDateComponents].year)&&(month == [self currentDateComponents].month)) {
        num = [self currentDateComponents].day-1;
    }
    switch (month) {
        case 1:
            return 31-num;
