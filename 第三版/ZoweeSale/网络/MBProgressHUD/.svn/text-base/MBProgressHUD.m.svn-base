...
//    [self performSelector:@selector(hideDelayed:) withObject:[NSNumber numberWithBool:animated] afterDelay:delay];
    //修改了延时的方法
    //原因:当tableview滑动的时候如果显示HUD会让HUD一直出现在屏幕上,延时的隐藏方法没有被调用
    NSTimer * timer = [NSTimer scheduledTimerWithTimeInterval:delay
                                                       target:self
                                                     selector:@selector(hideDelayed:)
                                                     userInfo:[NSNumber numberWithBool:animated]
                                                      repeats:NO];
    [[NSRunLoop currentRunLoop] addTimer:timer forMode:NSRunLoopCommonModes];
}
/**
 *  隐藏HUD
 *
 *  @param time 原方法是number类型,修改后animted从time中获取
 */
- (void)hideDelayed:(NSTimer *)time {
    
    NSNumber *animated = [time userInfo];
